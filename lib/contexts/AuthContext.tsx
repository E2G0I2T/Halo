...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const attemptAutoLogin = async () => {
    try {
      console.log("🔍 자동 로그인 시도..."); // 1. 기존 Google 세션 확인 (v13)

      const currentUser: GoogleUser | null =
        await GoogleSignin.getCurrentUser();
      if (currentUser && currentUser.idToken) {
        console.log("📱 기존 Google 계정 발견:", currentUser.user.email);
        await authenticateWithFirebase(currentUser.idToken);
        console.log("✅ 자동 로그인 성공 (캐시)");
        return true;
      } // 2. Silent Sign-In 시도 (v13)

      await GoogleSignin.hasPlayServices(); // 🔧 [수정] v13 반환 타입 처리 (제안해주신 로직)
      const silentResult: SignInSilentlyResponse =
        await GoogleSignin.signInSilently(); // 🔧 [수정] "data"가 있는지 확인 (성공 여부)
      if (silentResult && "data" in silentResult) {
        const silentUser = silentResult.data; // silentUser는 GoogleUser 타입
        console.log("📱 Silent 로그인 성공:", silentUser.user.email); // 🔧 [수정] getTokens()로 idToken 별도 요청
