// lib/hooks/useSongs.ts - 최종 완성 버전 (Auth 타이밍 이슈 해결)
...
export const useSongs = (authState?: AuthState): UseSongsReturn => {
  ...
  // 부드러운 업데이트 상태
  const [pendingRecommendationUpdate, setPendingRecommendationUpdate] = useState<PendingRecommendationUpdate>({
    isScheduled: false,
    countdown: 0,
    isCalculating: false
  });
  ...
  // 🆕 지연된 별점 변경 큐 (핵심 해결책)
  const [pendingRatingChanges, setPendingRatingChanges] = useState<PendingRatingChange[]>([]);
  ...
  const setOnRatingChangeCallback = useCallback((callback: RatingChangeCallback) => {
  ...
  const processPendingRatingChanges = useCallback(async () => {
    ...
    const significantChanges = pendingRatingChanges.filter(change => {
      const isSignificant = (
        ...
        (change.oldRating > 0 && change.newRating === 0) ||
        (change.oldRating > 0 && change.newRating > 0 && change.oldRating !== change.newRating)
      );
      return isSignificant;
    });

    if (significantChanges.length > 0) {
      console.log(`✨ 의미있는 지연된 변경 발견: ${significantChanges.length}건`);
  ...
  try {
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        const generatePersonalized = httpsCallable(functions, 'generatePersonalizedRecommendations');
        ...
        if (response && !response.isDefault) {
          if (response.recommendations?.personalizedOrder) {
        ...
        if (recommendations.length > 0) {
          console.log(`✅ ${attempt}차 시도 성공: ${recommendations.length}곡`);
  ...
  const onRatingChanged = useCallback((videoId: string, newRating: number, oldRating: number) => {
    ...
    if (!isAuthReady || !user?.uid) {
      ...
      setPendingRatingChanges(prev => {
        ...
        console.log(`📝 지연된 별점 변경 큐에 추가: ${videoId} (큐 크기: ${newQueue.length})`);
        return newQueue;
      });

      console.log(`⏳ Auth 준비 후 처리 예정: ${videoId} ${oldRating} → ${newRating}`);
      return;
    }
    ...
    if (pendingRecommendationUpdate.timeoutId) {
      console.log('🛑 기존 타이머 정리:', pendingRecommendationUpdate.timeoutId);
      clearTimeout(pendingRecommendationUpdate.timeoutId);
    }
    if (countdownTimerRef.current) {
      console.log('🛑 기존 카운트다운 타이머 정리:', countdownTimerRef.current);
  ...
  }, [user, isAuthReady, isFirebaseReady, authLoading, updateRecommendationsInBackground, pendingRecommendationUpdate.timeoutId]);
  ...
  useEffect(() => {
    ...
    console.log(`   pendingRatingChanges.length: ${pendingRatingChanges.length}`);
    
    if (isAuthReady && user?.uid && pendingRatingChanges.length > 0) {
      console.log('🔄 Auth 준비 완료 + 지연된 변경 있음 → 처리 시작');
      processPendingRatingChanges();
    }
  }, [isAuthReady, user?.uid, pendingRatingChanges.length, processPendingRatingChanges]);

  // 지연된 추천 업데이트 스케줄링
  const scheduleRecommendationUpdate = useCallback((delay: number = 3000) => {
    // 기존 스케줄 취소
    if (pendingRecommendationUpdate.timeoutId) {
      clearTimeout(pendingRecommendationUpdate.timeoutId);
    }
  ...
  const applyPendingRecommendations = useCallback(async () => {
    ...
    try {
      ...
      const sortedSongs = sortSongsByRecommendations(originalSongs, pendingRecommendationUpdate.newOrder);
  ...
  }, [pendingRecommendationUpdate.newOrder, originalSongs, cancelPendingRecommendations]);

  // 추천 데이터 로드
  const loadRecommendations = async (userId: string, forceRefresh: boolean = false): Promise<string[]> => {
    ...
    try {
      console.log(`🎯 추천 데이터 로드 시작: ${userId} (새로고침: ${forceRefresh})`);
      setIsLoadingRecommendations(true);

      const recommendations = await getUserRecommendations(userId, forceRefresh);
  ...
  // 곡 목록을 추천 순서로 정렬
  const applySongSorting = (rawSongs: Song[], recommendations: string[]): Song[] => {
  ...
  const loadInitialData = async () => {
    try {
      ...
      setErrorSafely(`데이터 로드 실패: ${error?.message || '알 수 없는 오류'}`);
      setLoading(false);
    }
  };
  ...
  const updateDataFromFirebase = async (ignoreCache: boolean = false) => {
    try {
      ...
      if (isAuthReady && user?.uid) {
        ...
        const recommendations = await loadRecommendations(user.uid, ignoreCache);
      ...
      console.log('✅ Firebase 데이터 업데이트 완료:', sortedSongs.length, '개');
      
    } catch (error: any) {
      console.error('❌ Firebase 업데이트 실패:', error);
      setErrorSafely(`데이터 업데이트 실패: ${error?.message || '알 수 없는 오류'}`);
    } finally {
      setIsUpdating(false);
    }
  };
